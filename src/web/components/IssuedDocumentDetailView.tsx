...
interface IssuedDocumentDetailViewProps {
  businessId?: string
  detail: AdminDocumentDetail | null
  onAttachmentsChanged?: (attachments: AdminDocumentAttachment[]) => void
}

export function IssuedDocumentDetailView({
  businessId,
  detail,
  onAttachmentsChanged,
}: IssuedDocumentDetailViewProps) {
  if (!detail) {
    return (
      <AdminEmptyState
        compact
  ...
  return (
    <div
      id="issued-document-print-sheet"
      className="issued-document-detail print-sheet dte-printable-shell"
    >
      <DtePrintableView detail={detail} />
      <div className="issued-document-detail__hero">
        <div className="document-detail-hero__content">
          <div className="document-detail-hero__chips">
            <span
              className={
                typeChipClassMap[
                  detail.document.documentType as IssuedDocumentFormValues['documentType']
                ]
              }
            >
              {
          ...
          <p>{detail.document.counterpartyName ?? 'Cliente no asociado'}</p>
          <small>
            {detail.document.counterpartyRut ?? 'Sin RUT'} · Documento emitido
          </small>
        </div>
        <span
          className={getStatusChipClass(
            detail.document.status as IssuedDocumentFormValues['status'],
          )}
        >
          {statusLabelMap[detail.document.status as IssuedDocumentFormValues['status']]}
        </span>
      </div>
      ...
      {detail.references[0] ? (
        <section className="issued-document-detail__reference document-detail-section">
          <div className="document-detail-section__header">
              ...
              <strong>
                {
                  typeLabelMap[
                    detail.references[0]
                      .referencedDocumentType as IssuedDocumentFormValues['documentType']
      ...
      <section className="issued-document-detail__notes document-detail-section">
        <div className="document-detail-section__header">
      ...
      <section className="issued-document-detail__lines document-detail-section">
        <div className="document-detail-section__header">
          <span>Lineas de detalle</span>
          <strong>{detail.lines.length} registros</strong>
        </div>
        {detail.lines.length ? (
          <div className="issued-document-detail__line-list">
            {detail.lines.map((line, index) => (
              <div key={line.id} className="issued-document-detail__line-item">
                <div>
