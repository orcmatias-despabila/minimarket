...
const productsTableName = import.meta.env.EXPO_PUBLIC_SUPABASE_PRODUCTS_TABLE || 'products'
  ...
  async saveProduct(input: SaveProductInput): Promise<SaveProductResult> {
    if (webSupabaseClient) {
      ...
      const query = input.existingProductId
        ? webSupabaseClient.from(productsTableName).update(payload).eq('id', input.existingProductId)
        : webSupabaseClient.from(productsTableName).insert(payload)
