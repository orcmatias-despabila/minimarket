...
interface BusinessMemberRecord {
  id: string
  business_id: string
...
const businessesTableName =
  import.meta.env.EXPO_PUBLIC_SUPABASE_BUSINESSES_TABLE || 'businesses'
const businessMembersTableName =
  import.meta.env.EXPO_PUBLIC_SUPABASE_BUSINESS_MEMBERS_TABLE || 'business_members'
const legacyBusinessMembersTableName =
  import.meta.env.EXPO_PUBLIC_SUPABASE_BUSINESS_MEMBERSHIPS_TABLE || 'business_memberships'
const businessInvitationsTableName =
  import.meta.env.EXPO_PUBLIC_SUPABASE_BUSINESS_INVITATIONS_TABLE || 'business_invitations'
...
const toFriendlyWorkspaceError = (message: string) => {
  const normalized = message.toLowerCase()

  if (
    normalized.includes('row-level security') &&
    (normalized.includes('business_members') || normalized.includes('business_memberships'))
  ) {
    return 'No pudimos unirte al negocio porque Supabase esta bloqueando la membresia por permisos RLS. Aplica la correccion SQL de invitaciones y vuelve a intentar.'
  }
...
export const webBusinessService = {
  ...
  async acceptInvitation(input: AcceptInvitationInput): Promise<WorkspaceSnapshot> {
    ...
    if (membershipInsert.error) {
      throw new Error(
        `No pudimos unirte al negocio: ${toFriendlyWorkspaceError(membershipInsert.error.message)}`,
      )
    }
  ...
  async rejectInvitation(input: RejectInvitationInput): Promise<void> {
    const client = ensureClient()
    const invitationResult = await client
      .from(businessInvitationsTableName)
      .select('id, business_id, email, status')
      .eq('id', input.invitationId)
      .maybeSingle<{ id: string; business_id: string; email: string; status: string }>()
    ...
    if (updateResult.error) {
      throw new Error(`No pudimos rechazar la invitacion: ${updateResult.error.message}`)
    }
  },
