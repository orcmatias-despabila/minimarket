...
interface SaleItemRecord {
  ...
  product_id: string
  product_name: string
  unit_measure: SaleItem['unitMeasure']
  quantity: number
  unit_price: number
...
const salesTableName = import.meta.env.EXPO_PUBLIC_SUPABASE_SALES_TABLE || 'sales'
const saleItemsTableName =
  import.meta.env.EXPO_PUBLIC_SUPABASE_SALE_ITEMS_TABLE || 'sale_items'
...
export const webSalesService = {
  ...
  async recordSale(input: PersistSaleInput): Promise<PersistSaleResult> {
    ...
    const saleInsert = await webSupabaseClient
      .from(salesTableName)
    ...
    if (sale.items.length) {
      const itemsInsert = await webSupabaseClient
